...
if ! have cryptsetup; then
    echo "$0 requires 'cryptsetup' to be installed!" | colorize ERROR
    exit 1
fi
if ! have sudo; then
    echo "$0 requires 'sudo' to be installed!" | colorize ERROR
    exit 1
fi
...
function makeContainer() {
    echo "Container '$1' doesn't exist; create it?" | colorize TITLE
    echo -n "(y/N): " | colorize PROMPT
    read answer
    echo
    ...
    case ${answer:0:1} in
        y|Y)
            ...
            echo "Use default options for encryption?" | colorize TITLE
            echo -n "(Y/n): " | colorize PROMPT
            read answer
            echo
            ...
            if [ ! -d $CONTAINER_DIR ]; then
                mkdir $CONTAINER_DIR
                sudo chown $WHOAMI $CONTAINER_DIR
                chmod 700 $CONTAINER_DIR
            fi

            # Make the container
            truncate -s $SIZE $CONTAINER_DIR/$1
            sudo chown $WHOAMI $CONTAINER_DIR/$1
            ...
            # Make the filesystem
            sudo mkfs.ext4 -L $1 /dev/mapper/$1
